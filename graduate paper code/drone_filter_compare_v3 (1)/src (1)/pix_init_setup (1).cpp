...
bool PXDroneControl::Init_Set(void) {
    ...
    P_EKF = 5.0 * Eigen::MatrixXf::Identity(6, 6);
    Q_EKF = 0.12 * Eigen::MatrixXf::Identity(6, 6);
    R_EKF = 25.0 * Eigen::MatrixXf::Identity(6, 6);

    Q_EKF_Init = 1.0 * Eigen::MatrixXf::Identity(6, 6);
    R_EKF_Init = 50.0 * Eigen::MatrixXf::Identity(6, 6);
    H_EKF = Eigen::MatrixXf::Identity(6, 6);
    X_EKF = Eigen::MatrixXf::Zero(6, 1);
    Z_EKF = Eigen::MatrixXf::Zero(6, 1);

    Adaptive_P_UKF = 5.0 * Eigen::MatrixXf::Identity(6, 6);
    P_UKF = 5.0 * Eigen::MatrixXf::Identity(6, 6);
    Q_UKF = 0.12 * Eigen::MatrixXf::Identity(6, 6);  // Q(6,6)=0.2;
    R_UKF = 25.0 * Eigen::MatrixXf::Identity(6, 6);
    ...
    W = Eigen::MatrixXf::Zero(2 * X_n * UKF_Number + 1, 1);
